...
/*!50001 VIEW `userorders` (`Заказ Но`,`Статус`,`Логин`,`Услуга`,`Скидка (%)`,`Сумма Заказа`,`Итого`) AS select `o`.`id` AS `id`,`o`.`status` AS `status`,`o`.`userLogin` AS `userLogin`,`s`.`name` AS `name`,`o`.`discountPercent` AS `discountPercent`,`s`.`price` AS `price`,round((`s`.`price` * (1 - (`o`.`discountPercent` / 100))),2) AS `totalAmount` from (`orders` `o` join `services` `s`) where (`o`.`serviceId` = `s`.`id`) order by `o`.`id` */;
